	...
	function showMessages(pageNo, pageSize) {
		$.get("${pageContext.request.contextPath}/msg.do", {
			...
			for(var i = 0;i<data.msgList.length;i++){
				...
				var $em2=$("<em><a href='${pageContext.request.contextPath}/user.do?param=queryallusers&sendid="+data.msgList[i].sendid +"'>回信</a></em>");
				...
				if(data.msgList[i].msgcontent.length>8){
					$a=$("<a href='${pageContext.request.contextPath}/msg.do?param=showmsgbyid&id="+data.msgList[i].id+"'>"+data.msgList[i].msgcontent.substring(0,8)+"</a>");
				}else{
					$a=$("<a href='${pageContext.request.contextPath}/msg.do?param=showmsgbyid&id="+data.msgList[i].id+"'>"+data.msgList[i].msgcontent+"</a>");
				}
